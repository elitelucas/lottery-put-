...
const BankPage = (props) => {
  const [data,setData]=useState(JSON.parse(localStorage.getItem('auth')).user.bank_card);
  const [addData,setAddData]=useState({state:false,actual_name:'',ifsc_code:'',bank_name:'',bank_account:'',state_territory:'',
    ...
    if(addData.actual_name!='' && addData.ifsc_code!=='' && addData.bank_name!=='' && addData.bank_account!=='' && addData.state_territory!=='' && addData.city!=='' && addData.address!=='' && addData.mobile_number!=='' && addData.email!=='')
    {
      (async ()=>{
        ...
        if(response.status==200){
          ...
          tmp.push(addData);
          var tmp1=JSON.parse(localStorage.getItem('auth'));
          tmp1.user.bank_card=tmp;
          localStorage.setItem('auth',JSON.stringify(tmp1));
          setData(tmp);
          setAddData({state:false,actual_name:'',ifsc_code:'',bank_name:'',bank_account:'',state_territory:'',
          city:'',address:'',mobile_number:'',email:'',upi_account:''});
  ...
  // },[]);
  return (
    <Page title={(<><Link to="/my"><Typography type="h4" className={bem.e('title')}><FaArrowCircleLeft /> Bank</Typography></Link><Button color="link" onClick={()=>{setAddData({...addData,state:true})}} style={{"padding":"20px"}}><FaPlus /></Button></>)} className="MyPage"  >
      ...
      <Col xl={12} lg={12} md={12}>   
        {
          data ? data.map((ele,key)=>(
            ...
            <div key={key} className={'form-control'} style={{ marginBottom: '0rem' }}><span onClick={()=>setToggle(key)}><FaWallet className="mr-3" /> {ele.actual_name}-{ele.bank_account}</span> <span onClick={remove(key)}><FaWindowClose style={{float:'right'}} className="ml-3" /></span></div>

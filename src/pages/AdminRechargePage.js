...
const AdminRechargePage = (props) => {
  ...
  return (
    <Page title={(<><Link to="/my"><Typography type="h4" className={bem.e('title')}><FaArrowCircleLeft /> Admin Recharge</Typography></Link></>)} className="MyPage" 
    >      
      <Row>   
        <Col md={12}>
        {
          ...
          list.map((ele,key)=>(
            ...
            {ele.orderID}
            <br />  {ele.createdAt}
            <span style={{float:'right'}}>{ele.status==1 ? (<FaMoneyCheck style={{color:'blue'}} />) : 
              (ele.status==0 ? (<FaHourglassHalf style={{color:'gold'}} />)
                  : (<FaRegTimesCircle style={{color:'red'}} />))}</span></Button>
          )) : ''
        }    
